...
public class SortedLinkedSet<T> extends AbstractSortedSet<T> implements SortedSet<T> {
     ...
     */
    @Override
    public void insert(T element) { // WHAT ABOUT THE LAST ELEMENT INSERTION ??????????? HW
    ...
    public void delete(T element) {
        Finder finder = new Finder(element);
        if (finder.found){
            if (finder.previous == null){ // the first element should be deleted
                first = first.next;
            } else { // the normal deletion (also at the end)
    ...
    public static <T> SortedLinkedSet<T> union(SortedSet<T> sortedSet1, SortedSet<T> sortedSet2) {
        ...
        while (element1 != null && element2 != null){ // while we have values in each set
        ...
        while (element1 != null) { // add the remaining elements from set1
            builder.append(element1);
            element1 = nextOrNull(iterator1);
        }
        while (element2 != null) { // add the remaining elements from set2
            builder.append(element2);
            element2 = nextOrNull(iterator2);
        }
    ...
    public static <T> SortedLinkedSet<T> intersection(SortedSet<T> sortedSet1, SortedSet<T> sortedSet2) {
        ...
        // in the worst case the complexity is O(n+m) where n and m are number of elements in each set

        while (element1 != null && element2 != null){ // while we have svalue from each set
            int cmp = comparator.compare(element1, element2);
