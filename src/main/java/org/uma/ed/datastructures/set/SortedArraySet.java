...
public class SortedArraySet<T> extends AbstractSortedSet<T> implements SortedSet<T> {
    ...
    public void insert(T element) {
        ...
        if (!finder.found){ // if the element is found the insertion is ignored
          ensureCapacity(); // O(n)
          for (int i = size; i > finder.index; i--) { // O(n)
    ...
    public void delete(T element) {
        Finder finder = new Finder(element); // O(log n)
        if (finder.found){ // if it is not found the deletion is ignored
            for (int i = finder.index; i < size - 1; i++) { // O(n)
                elements[i] = elements[i + 1];
            }
            size--;
            elements[size] = null;  // IMPORTANT
        }
    }
