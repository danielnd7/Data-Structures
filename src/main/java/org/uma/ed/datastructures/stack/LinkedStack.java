...
public class LinkedStack<T> extends AbstractStack<T> implements Stack<T> {
     ...
     */
    public static <T> LinkedStack<T> copyOf(LinkedStack<T> that) { // CORRECT? ???????????????????????????
    // An auxiliary copy is used because in order to keep the original order it is necessary to copy two times.
        LinkedStack<T> auxCopy = new LinkedStack<>();
        LinkedStack<T> copy = new LinkedStack<>();

        Node<T> nextNode = that.top;
        while (nextNode != null) {
            auxCopy.push(nextNode.element);
            nextNode = nextNode.next;
        }

        while (!auxCopy.isEmpty()){
          copy.push(auxCopy.top.element);
          auxCopy.pop();
        }
     ...
     * @return a new LinkedStack with same elements and order as {@code that}.
     */
    public static <T> LinkedStack<T> copyOf(Stack<T> that) { // CORRECT? ???????????????????????????
        LinkedStack<T> copy = new LinkedStack<>();
        LinkedStack<T> auxCopy = new LinkedStack<>();


        while (!that.isEmpty()){
            auxCopy.push(that.top());
            that.pop();
        }

        while (!auxCopy.isEmpty()){
            that.push(auxCopy.top());
            copy.push(auxCopy.top());
            auxCopy.pop();
        }
